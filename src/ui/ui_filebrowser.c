...
static void build_path(char *out, size_t size, const char *dir,
                       const char *name) {
  if (strcmp(dir, "/SD:") == 0) {
...
void ui_filebrowser_set_callback(ui_filebrowser_cb_t cb) { rom_select_cb = cb; }
...
const char *ui_filebrowser_get_path(void) { return current_path; }
