...
lv_obj_t *ui_game_create(lv_obj_t *parent) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    lv_obj_align(btn, dpad_btns[i].align, dpad_btns[i].x_offs,
                 dpad_btns[i].y_offs);
    ...
    lv_obj_add_event_cb(btn, on_btn_pressed, LV_EVENT_PRESSED,
                        (void *)(uintptr_t)dpad_btns[i].mask);
    lv_obj_add_event_cb(btn, on_btn_released, LV_EVENT_RELEASED,
                        (void *)(uintptr_t)dpad_btns[i].mask);
  }
  ...
  lv_canvas_set_buffer(canvas, canvas_buf, NES_CANVAS_W, NES_CANVAS_H,
  ...
  for (int i = 0; i < 4; i++) {
    ...
    lv_obj_align(btn, LV_ALIGN_CENTER, action_btns[i].x_offs,
                 action_btns[i].y_offs);
    ...
    lv_obj_add_event_cb(btn, on_btn_pressed, LV_EVENT_PRESSED,
                        (void *)(uintptr_t)action_btns[i].mask);
    lv_obj_add_event_cb(btn, on_btn_released, LV_EVENT_RELEASED,
                        (void *)(uintptr_t)action_btns[i].mask);
  }
...
void ui_game_update_debug_bar(const struct nes_system *sys,
  ...
  snprintk(buf, sizeof(buf),
           "FPS:%.1f | Mapper:%d | PRG:%dKB | CHR:%dKB | Frame:%u", sys->fps,
           sys->cart.mapper_num, (int)(sys->cart.prg_rom_size / 1024),
           (int)(sys->cart.chr_size / 1024), sys->frame_count);
...
void ui_game_set_ctrl_callback(ui_game_ctrl_cb_t cb) { ctrl_cb = cb; }

void ui_game_set_btn_callback(ui_game_btn_cb_t cb) { btn_cb = cb; }
