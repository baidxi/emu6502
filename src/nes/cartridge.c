...
int cart_load_rom(nes_cart_t *cart, const uint8_t *data, size_t size,
                  const char *path) {
  ...
  if (data[0] != 'N' || data[1] != 'E' || data[2] != 'S' || data[3] != 0x1A) {
    LOG_ERR("Invalid iNES header: %02X %02X %02X %02X", data[0], data[1],
            data[2], data[3]);
    return -EINVAL;
  }
  ...
  LOG_INF("ROM parsed: Mapper=%d PRG=%dKB CHR=%dKB Battery=%d Mirror=%d",
          cart->mapper_num, (int)(cart->prg_rom_size / 1024),
          (int)(cart->chr_size / 1024), cart->battery, cart->mirror);
