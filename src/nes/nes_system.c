...
int nes_system_load_rom(nes_system_t *sys, const uint8_t *data, size_t size,
                        const char *path) {
  /* 释放之前的 ROM */
...
int nes_system_run_frame(nes_system_t *sys) {
  ...
  if (first_frame_diag) {
    if (0)
      LOG_ERR("=== Frame start: PC=%04X MASK=%02X CTRL=%02X ===", sys->cpu.pc,
              sys->ppu.mask, sys->ppu.ctrl);
  ...
  for (int scanline = 0; scanline < SCANLINES_PER_FRAME; scanline++) {
    ...
    if (sys->frame_count <= 1 && (scanline % 50) == 0) {
      if (0)
        LOG_ERR("  scanline %d/%d PC=%04X", scanline, SCANLINES_PER_FRAME,
                sys->cpu.pc);
    }
    ...
    /* VBlank NMI: 在扫描线241开始时立即触发 (先PPU设置VBlank, 再触发NMI,
     * 然后CPU) */
    if (scanline == 241) {
    ...
    while (cpu_cycles_accum < cpu_cycles_target) {
      ...
      if (instr_count > max_instr_per_scanline) {
        LOG_ERR("CPU stuck: %d instr on scanline %d, PC=%04X", instr_count,
                scanline, sys->cpu.pc);
  ...
  if (!ppu_disabled_warned && sys->frame_count >= 3 &&
      !(sys->ppu.mask & 0x18)) {
    if (0)
      LOG_WRN("Frame %u: PPU rendering still disabled (MASK=%02X CTRL=%02X)",
